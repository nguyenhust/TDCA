...
{
    public partial class DropDownGiangDuong : RadMultiColumnComboBox
    {
        HC_GiangDuong_Phong_Coll listItem;
        public DropDownGiangDuong()
        {
            InitializeComponent();
            this.Text = string.Empty;
            this.NullText = "Chọn giảng đường - phòng họp";
        }

        private void BindData()
        {
            string columnVisible = "TenPhong";
            string columnVisible2 = "NoiDung";
            string columnVisible3 = "DiaDiem";
            ...
            gridViewControl.Columns[columnVisible].HeaderText = "Phòng";
            gridViewControl.Columns[columnVisible2].IsVisible = true;
            gridViewControl.Columns[columnVisible2].HeaderText = "Sức chứa";
            gridViewControl.Columns[columnVisible3].IsVisible = true;
            gridViewControl.Columns[columnVisible3].HeaderText = "Địa điểm";
            gridViewControl.MasterTemplate.AutoGenerateColumns = false;
            gridViewControl.MasterTemplate.AutoSizeColumnsMode = GridViewAutoSizeColumnsMode.Fill;
        ...
        {
            listItem = HC_GiangDuong_Phong_Coll.GetHC_GiangDuong_Phong_Coll(new BusinessFunction(BusinessFunctionMode.GetDataForDropDownWithCondition));
            BindData();
        }
        ...
        public void FillData(HC_GiangDuong_Phong_Coll chucVu)
        {
            this.listItem = chucVu;
            BindData();
        }
        ...
        /// </summary>
        /// <returns></returns>
        public HC_GiangDuong_Phong_Coll GetListData()
        {
            return listItem;
        }
        /// <summary>
        ...
        public object Selected_TextValue
        {
            get
            {
                try
                {
                    return listItem[this.SelectedIndex].TenPhong;
                }
                catch
                {
                    return null;
            ...
            set
            {
                try
                {
                    for (int i = 0; i < listItem.Count; i++)
                    {
                        if (listItem[i].TenPhong == value.ToString())
                        {
                            this.SelectedIndex = i;
                        }
                    }
