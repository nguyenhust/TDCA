...
{
    public partial class Grid_HC_CongVanDi : UsDictionary
    {
        #region variables
        ...
        {
            return TextMessages.ControlID.HC_CongVanDen;
        }

        public override string ToString()
        {
            return TextMessages.FormTitle.HC_CongVanDen;
        }

        protected override void ApplyAuthorizationRules()
        {
            bl_btnSave = GlobalCommon.IsHavePermission(TextMessages.RolePermission.HC_CongVanDen_Insert);
            //Nếu không sử dụng chức năng in thì Role sẽ check là View. còn nếu sử dụng thì role là Print
            bl_btnPrint = GlobalCommon.IsHavePermission(TextMessages.RolePermission.HC_CongVanDen_Print);
            bl_btnMyClose = true;
            bl_btnDelete = GlobalCommon.IsHavePermission(TextMessages.RolePermission.HC_CongVanDen_Del);
        }

        protected override void Save()
        {
            formMode.IsInsert = true;
            Form_CongVan_Di fr = new Form_CongVan_Di(formMode);
            fr.ShowDialog();
            LoadUS();
        }
        ...
        protected override void LoadUS()
        {
            try
            {
                ...
                {
                    HC_CongVanDi_Coll listTemp = HC_CongVanDi_Coll.GetHC_CongVanDi_Coll(new BusinessFunction(BusinessFunctionMode.GetDataForGridView,PTIdentity.IDNguoiDung));
                    HC_CongVanDi_Coll listItem = HC_CongVanDi_Coll.NewHC_CongVanDi_Coll();
                    int LastID = -1;
                    foreach(HC_CongVanDi_Info itemInfo in listTemp)
                    {
                        if (itemInfo.ID != LastID)
                        {
                            listItem.Add(itemInfo.Clone());
                            LastID = itemInfo.ID;
                        }
                    }
        ...
        protected override void Delete()
        {
            try
            {
                ...
                {
                    HC_CongVanDi.DeleteHC_CongVanDi(selectedID);
                    LoadUS();
                }
            }
            catch (Exception ex) { GlobalCommon.ShowErrorMessager(ex); }
        ...
        private void radGridView_CellDoubleClick(object sender, Telerik.WinControls.UI.GridViewCellEventArgs e)
        {
            try
            {
                if (int.TryParse(radGridView.CurrentRow.Cells["Id"].Value.ToString(), out selectedID)
                    && GlobalCommon.IsHavePermission(new string[]{TextMessages.RolePermission.HC_CongVanDen_Edit,TextMessages.RolePermission.HC_CongVanDen_View}))
                {
        ...
        private void PrintToHTML()
        {
            ...
            cv.E_TieuDe = new List<string>() { "Số công văn", "Ngày ban hành", "Về việc", "Nơi gửi", "Vị trí lưu"};
            ...
            cv.E_TieuDeBaoCao = "báo cáo công văn đến";
