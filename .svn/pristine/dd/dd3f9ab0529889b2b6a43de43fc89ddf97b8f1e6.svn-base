...
namespace DataMigrate
{
    public partial class Form1 : Form
    {
        ...
        Dictionary<string, int> Li_ChuyenKhoaStr = new Dictionary<string, int>();
        ...
        Dictionary<string, long> Li_DMBenhVienStr = new Dictionary<string, long>();
        Dictionary<string, int> Li_ChucVuTDC = new Dictionary<string, int>();
        ...
        private void MappingKhungLopHoc()
        {
            ...
            string[] RlistFrom = new string[] { "RHM", "rhm" };
            string[] RlistTo = new string[] { "răng hàm mặt", "răng hàm mặt" };
            ...
            foreach (DM_LOPHOC_INFO o in oldI)
            {
                ...
                if (!done)
                {
                    try
                    {
                        ...
                        string tenlop, khoa;
                        ProcessTenLopHoc(o.TEN_LH,out tenlop,out khoa);
                        tenlop = ProcessReplaceStrNotLower(tenlop, RlistFrom, RlistTo);
                        itemx.TenLop = tenlop;
                        itemx.ApplyEdit();
                        itemx = itemx.Save();
                        Li_DMLopHoc.Add(o.DM_LOPHOC_ID, itemx.Id);
                        newI = DT_LienTuc_KhungLopHoc_Coll.GetDT_LienTuc_KhungLopHoc_Coll();
        ...
        private void MappingChuyenNganh()
        {
            ...
            Li_ChuyenKhoaStr = new Dictionary<string, int>();
            ...
            string[] RlistFrom = new string[] { "tt", "TT" };
            string[] RlistTo = new string[] { "Trung tâm", "Trung tâm" };
            ProTask.Maximum = oldI.Count;
            ProTask.Value = 0;
            bool done = false;
            ...
            {
                {
                    foreach (DIC_ChuyenKhoa_Info n in newI)
                        if (ProcessReplaceStr(o.TEN_CHUYENNGANH, Rlist) == ProcessReplaceStr(n.Ten, Rlist))
                        {
                            ...
                            Li_ChuyenKhoaStr.Add(o.TEN_CHUYENNGANH, n.ID);
                            //result.Add(o);
                            done = true;
                            break;
                        }
                ...
                if (!done)
                {
                    ...
                    item.Ten = ProcessReplaceStrNotLower(o.TEN_CHUYENNGANH,RlistFrom,RlistTo);
                    ...
                    Li_ChuyenKhoaStr.Add(o.TEN_CHUYENNGANH, item.ID);
                }
                done = false;
                ProTask.Value++;
            }
        ...
        {
            Li_TrinhDo2 = new Dictionary<string, int>();
            Li_TrinhDo2.Add(string.Empty, 0);
            CTAC_TUYEN_CTIET_COLL ol = CTAC_TUYEN_CTIET_COLL.GetCTAC_TUYEN_CTIET_COLL();
            DIC_HocVi_Coll nl = DIC_HocVi_Coll.GetDIC_HocVi_Coll();
            CTAC_TUYEN_CTIET_COLL Err = CTAC_TUYEN_CTIET_COLL.NewCTAC_TUYEN_CTIET_COLL();
            CTAC_TUYEN_CTIET_COLL Re = CTAC_TUYEN_CTIET_COLL.NewCTAC_TUYEN_CTIET_COLL();
            ProTask.Maximum = ol.Count;
            ProTask.Value = 0;
            bool done = false;
            string[] listF = new string[] { "bsck", "sinh hóa", "sỹ", "th" };
            string[] listT = new string[] { "bác sĩ chuyên khoa", "","sĩ","trung học" };
            foreach (CTAC_TUYEN_CTIET_INFO o in ol)
            {
                foreach (DIC_HocVi_Info n in nl)
                {
                    if (ProcessReplaceStr(o.TRINH_DO, listF, listT) == ProcessReplaceStr(n.TenHocVi, listF, listT))
                    {
                        Re.Add(o);
                        try
                        {
                            Li_TrinhDo2.Add(o.TRINH_DO, n.ID);
                            done = true;
                            break;
                        }
                        catch
           ...
           // radGridView1.DataSource = Re;
           // radGridView2.DataSource = Err;
        }

        private void MappingChuVu_ForCDT()
        {
            Li_ChucVuTDC = new Dictionary<string, int>();
            Li_ChucVuTDC.Add(string.Empty, 0);
            CTAC_TUYEN_CTIET_COLL ol = CTAC_TUYEN_CTIET_COLL.GetCTAC_TUYEN_CTIET_COLL();
            DIC_ChucVu_Coll nl = DIC_ChucVu_Coll.GetDIC_ChucVu_Coll();
            CTAC_TUYEN_CTIET_COLL Err = CTAC_TUYEN_CTIET_COLL.NewCTAC_TUYEN_CTIET_COLL();
            CTAC_TUYEN_CTIET_COLL Re = CTAC_TUYEN_CTIET_COLL.NewCTAC_TUYEN_CTIET_COLL();
            ProTask.Maximum = ol.Count;
            ProTask.Value = 0;
            bool done = false;
            string[] listF = new string[] { "phó trưởng khoa", "cán bộ"};
            string[] listT = new string[] { "phó khoa" ,"nhân viên"};
            foreach (CTAC_TUYEN_CTIET_INFO o in ol)
            {
                foreach (DIC_ChucVu_Info n in nl)
                {
                  
                    if (ProcessReplaceStr(o.CHUC_VU, listF, listT) == ProcessReplaceStr(n.Ten, listF, listT))
                    {
                        Re.Add(o);
                        try
                        {
                            Li_ChucVuTDC.Add(o.CHUC_VU, n.ID);
                            done = true;
                            break;
                        }
                        catch
                ...
                {
                    Err.Add(o);
                    if (!string.IsNullOrEmpty(o.CHUC_VU))
                    {
                        DIC_ChucVu item = DIC_ChucVu.NewDIC_ChucVu();
                        item.Ten = o.CHUC_VU;
                        item.SuDung = true;
                        item.ApplyEdit();
                        item = item.Save();
                        nl = DIC_ChucVu_Coll.GetDIC_ChucVu_Coll();
                        Li_ChucVuTDC.Add(o.CHUC_VU, item.ID);
                    }
                    done = false;
                }
                done = false;
            ...
            foreach (MA_BVIEN_INFO o in olI)
            {
                foreach (DIC_BenhVien_Info n in newI)
                {
                    if (ProcessReplaceStr(o.TEN_BVIEN.ToLower().Trim(), Re) == ProcessReplaceStr(n.Ten.ToLower().Trim(), Re))
                    {
                        try
                        {
                            ...
                            Li_DMBenhVienStr.Add(o.TEN_BVIEN.ToLower().Trim(), n.ID);
                        }
                        catch
                        {
                        }
                ...
                if (!done)
                {
                    ...
                    try
                    {
                        ...
                        Li_DMBenhVienStr.Add(o.TEN_BVIEN.ToLower().Trim(), item.ID);
                    }
                    catch
                    {
                    }
        ...
        private void MappingNoiCongTacHV()
        {
            HOC_VIEN_TT_COLL olI = HOC_VIEN_TT_COLL.GetHOC_VIEN_TT_COLL();
            DIC_BenhVien_Coll newI = DIC_BenhVien_Coll.GetDIC_BenhVien_Coll();
            HOC_VIEN_TT_COLL Err = HOC_VIEN_TT_COLL.NewHOC_VIEN_TT_COLL();
            HOC_VIEN_TT_COLL R = HOC_VIEN_TT_COLL.NewHOC_VIEN_TT_COLL();
            ProTask.Maximum = olI.Count;
            ProTask.Value = 0;
            Li_DMBenhVien = new Dictionary<int, long>();
            bool done = false;
            string[] Re = new string[] { "bệnh viện", "viện" };
            foreach (HOC_VIEN_TT_INFO o in olI)
            {
                foreach (DIC_BenhVien_Info n in newI)
                {
                    if (ProcessReplaceStr(o.NOI_CTAC.ToLower().Trim(), Re) == ProcessReplaceStr(n.Ten.ToLower().Trim(), Re))
                    {
                        try
                        {
                            Li_DMBenhVienStr.Add(o.NOI_CTAC, n.ID);
                        }
                        catch
                        {
                        }
                ...
                if (!done)
                {
                   ...
                   // item = item.Save();
                    ...
                    try
                    {
                        Li_DMBenhVienStr.Add(o.NOI_CTAC, item.ID);
                    }
                    catch
                    {
                    }
        ...
        private void CreateLopHoc()
        {
            ...
            foreach (LOP_HOC_INFO itemInfo in itemColl)
            {
                try
                {
                    ...
                    string tenlop, khoahoc;
                    ProcessTenLopHoc(itemInfo.TEN_LOP, out tenlop, out khoahoc);
                    newItem.TenLop = tenlop;
                    newItem.KhoaHoc = khoahoc;
                    newItem.MaLop = itemInfo.MA_LOP;
                    if (newItem.MaLop == "TL-BM-B24-CĐHA-13-06")
                    {
                    }
                    ...
                    try
                    {
                        if (itemInfo.DM_LOPHOC_ID == 2)
                        {
                        }
                        newItem.IdKhungLopHoc = Li_DMLopHoc[Convert.ToInt32(itemInfo.DM_LOPHOC_ID)];
                        DT_LienTuc_KhungLopHoc itemxxx = DT_LienTuc_KhungLopHoc.GetDT_LienTuc_KhungLopHoc(Convert.ToInt32(newItem.IdKhungLopHoc));
                        newItem.TenLop = itemxxx.TenLop;
                    }
                    catch
                    {
                  ...
                  //  newItem.MaLop = ProTask.Value.ToString();
                    ...
                    if (!string.IsNullOrEmpty(newItem.MaLop))
                    {
                        for (int i = 10; i < 15; i++)
                        {
                            if (newItem.MaLop.IndexOf(string.Format("-{0}-", i)) > 0)
                            {
                                try
                                {
                                    newItem.Backup06 = Convert.ToInt32(newItem.MaLop.Substring(newItem.MaLop.IndexOf(string.Format("-{0}-", i)) + 4));
                                    break;
                                }
                                catch
                                {
        ...
        private void BindDiaDiemLopHoc()
        {
            LOP_HOC_CTIET_COLL od = LOP_HOC_CTIET_COLL.GetLOP_HOC_CTIET_COLL();
            ProTask.Maximum = od.Count;
            ProTask.Value = 0;
            foreach (LOP_HOC_CTIET_INFO o in od)
            {
                DT_LienTuc_LopHoc_DiaDiemToChuc nI = DT_LienTuc_LopHoc_DiaDiemToChuc.NewDT_LienTuc_LopHoc_DiaDiemToChuc();
                nI.MaLop = o.MA_LOP;
                nI.TuNgay = o.TU_NGAY;
                nI.DenNgay = o.DEN_NGAY;
                nI.DiaDiem = o.TenCQ;
                nI.DonViThamGia = o.TenCK;
                try
                {
                    nI.IdBenhVien = Li_DMBenhVienStr[o.TenCQ.ToLower().Trim()];
                }
                catch
                {
                }
                //nI.DonViThamGia = o.TenCQ;
                try
                {
                    nI.ApplyEdit();
                    nI.Save();
        ...
        private void CreateHocVien()
        {
            ...
            foreach (HOC_VIEN_TT_INFO itemInfo in oldI)
            {
                ...
                if (itemInfo.HINH_THUC.ToLower().Trim() != "k")
                {
                    try
                    {
                        nItem.MaLopHoc = Li_MaLopHoc[Convert.ToInt32(itemInfo.LOP_HOC_ID)];
                ...
                try
                {
                    nItem.IdBenhVienCongTac = Li_DMBenhVienStr[itemInfo.NOI_CTAC.ToLower().Trim()];
                    
                   // nItem.IdBenhVienCongTac = Li_DMBenhVien[
                }
                catch
                {
                    nItem.NoiCongTac = itemInfo.NOI_CTAC;
                }
        ...
        private void CreateCanBoDiTuyen()
        {
            ...
            foreach (CTAC_TUYEN_CTIET_INFO i in ol)
            {
                ...
                try
                {
                    ni.IdChuyenNganh = Li_TrinhDo2[i.TRINH_DO];
                }
                catch
                {
                }
                try
                {
                    ni.IdChuyenKhoa = Li_ChuyenKhoaStr[i.TenBP];
                }
                catch
                {
                ...
                try
                {
                    ni.IdChucVu = Li_ChucVuTDC[i.CHUC_VU];
                }
                catch
                {
                }
        ...
        private void ProcessTenLopHoc(string FName,out string LName, out string KhoaLop)
        {
            string[] L = new string[] { "CGKT", "Luxemburgh", "khóa I", "Vinh Nghệ An", "Bắc Trung Bộ", "Nam Trung Bộ", "BCA", "Bộ CA", "VIE", "(Bộ CA)", "(XHH+BCA)", "Sơn La", "Bắc Ninh", "Bắc Giang", "Ninh Bình", "Thái Nguyên" };
            string[] R = new string[] { ":" };
            KhoaLop = string.Empty;
            foreach (string l in L)
            {
                if (FName.IndexOf(l) >= 0)
                {
                    FName = FName.Replace(l, string.Empty);
                    KhoaLop += l;
                }
            }
            int x;
            for (int i = 0; i < 100; i++)
            {
                ...
                if (x >= 0)
                {
                    ...
                    KhoaLop += FName.Substring(x - 1, FName.Length - x + 1);
            ...
            {
                FName = FName.Replace(r, string.Empty);
            }
            LName = FName.Trim();
        }
        ...
        private string ProcessReplaceStrNotLower(string str, string[] RListFrom, string[] RListTo)
        {
            if (RListFrom.Length != RListTo.Length)
                return string.Empty;
            //str = str.ToLower();
            for (int i = 0; i < RListFrom.Length; i++)
            {
                str = str.Replace(RListFrom[i], RListTo[i]);
            }
        ...
        {
            string[] L = new string[] { "CGKT:", "Luxemburgh", "khóa I", "Vinh Nghệ An", "Bắc Trung Bộ", "Nam Trung Bộ", "BCA", "Bộ CA", "VIE", "(Bộ CA)", "(XHH+BCA)", "Sơn La", "Bắc Ninh", "Bắc Giang", "Ninh Bình", "Thái Nguyên","- NTB" };
            dauvao += " ";
            for (int i = 0; i < 101; i++)
            {
                dauvao = dauvao.ToLower().Replace(string.Format(" k{0} ", i.ToString()), string.Empty);
                dauvao = dauvao.ToLower().Replace(string.Format(" k {0} ", i.ToString()), string.Empty);
                dauvao = dauvao.ToLower().Replace(string.Format(" khóa {0} ", i.ToString()), string.Empty);
            }
            foreach (string l in L)
            {
                dauvao = dauvao.Replace(l.ToLower(), string.Empty);
            }
